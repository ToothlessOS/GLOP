...
from rbf_soft_clustered_generator import generate_rbf_soft_contracted_batch

"""
Modified data generator based on Liming's rbf_soft_clustered_generator.py
(Changed) def generate_tsp_data(dataset_size, opts)
...
def solve_and_decompose(instance, opts):
    ...
    for shift in range(revision_iter):
        decomposed_seeds, offset_seed = decomposition(
            seed, coordinate_dim, revision_len, offset, shift
        )
        if dataset is None:
...
def generate_tsp_data(dataset_size, opts):
    ...
    # Importantly, this is aligned with the shape of the rbf soft generator
    # So we can directly apply the rbf soft perturbation afterwards!

    gen = torch.Generator(device="cpu")
    gen.manual_seed(opts.seed)

    clustered, metadata = generate_rbf_soft_contracted_batch(
        dataset, generator=gen, alpha=0.8, rho=0.35  # default
    )

    print("rbf-soft summary:", metadata.summary())
    print("dataset shape:", clustered.shape)
    return clustered
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir",
        default="data/RI_w_rbf_soft_train_tsp",
        help="Create datasets in data_dir",
    )
    parser.add_argument(
        "--name", default="RI_w_rbf_soft", type=str, help="Name to identify dataset"
    )
    parser.add_argument(
        "--dataset_size", type=int, default=1000000, help="Size of the dataset"
    )
    parser.add_argument(
        "--graph_size", type=int, default=500, help="Sizes of problem instances"
    )
    ...
    filename = os.path.join(
        datadir,
        "{}_{}{}_seed{}.pt".format(graph_size, opts.name, opts.revision_len, opts.seed),
    )
