...
def LCP_TSP(
    ...
    layer_start = time.time()  # per-layer wall-clock start
    for i in range(revision_iter):
        ...
        # Per-iteration logging — closed-loop tour cost across all --width restarts.
...
def reconnect(
    ...
    for revision_id in range(len(revisers)):
        ...
        # Layer-end summary — best/avg across --width restarts, aggregated over eval batch
